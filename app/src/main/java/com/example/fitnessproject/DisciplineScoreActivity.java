...
public class DisciplineScoreActivity extends AppCompatActivity {
    ...
    private void loadData() {
        ...
        int total = 0;
        try {
            total = new JSONArray(prefs.getString("workoutHistory", "[]")).length();
        } catch (Exception ignored) {}

        // Completion rate = (completed / total exercises planned)
        int completed = prefs.getInt("workoutCompleted", 0);
        int totalPlan = prefs.getInt("workoutTotal", 6);
